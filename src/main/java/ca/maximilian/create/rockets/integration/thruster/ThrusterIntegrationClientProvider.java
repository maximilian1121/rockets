...
public class ThrusterIntegrationClientProvider implements IBlockComponentProvider {
    ...
    public void appendTooltip(ITooltip tooltip, BlockAccessor accessor, IPluginConfig config) {
        ...
        var totalTicks =
            accessor.getServerData()
                .getInt(ThrusterIntegrationHelper.TOTAL_FUEL_TICKS_IDENTIFIER);
        if (totalTicks <= 0) {
    ...
    private void showFuelIcon(ITooltip tooltip, @NotNull AbstractThrusterBlockEntity thruster) {
        var fuelStack = thruster.getFuelContainer().getItem(0);
        if (!fuelStack.isEmpty()) {
            tooltip.add(IElementHelper.get().item(fuelStack));
        }
    }
}
