...
class ThrusterIntegrationHelper {
    ...
    static void appendFuelData(
            final CompoundTag data, final @NotNull AbstractThrusterBlockEntity thruster) {
    ...
    static @Nullable AbstractThrusterBlockEntity resolveThruster(
            final @NotNull BlockAccessor accessor) {
        ...
        if (state.getBlock() instanceof ThrusterBlock thrusterBlock) {
            return thrusterBlock.getBaseBlockEntity(
                    accessor.getLevel(), state, accessor.getPosition());
        }
