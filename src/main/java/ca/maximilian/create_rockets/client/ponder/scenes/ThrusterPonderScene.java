package ca.maximilian.create_rockets.client.ponder.scenes;
...
public class ThrusterPonderScene {
    ...
    public static void thruster(final SceneBuilder builder,
                                final @NotNull SceneBuildingUtil util) {
    ...
    private static void addFuelSubScene(@NotNull CreateSceneBuilder scene,
                                        @NotNull Selection thrusterSelection,
                                        CreateSceneBuilder.@NotNull WorldInstructions world) {
        scene.addKeyframe();

        scene.overlay().showControls(
                thrusterSelection.getCenter(),
                Pointing.DOWN,
                60
        ...
        world.modifyBlockEntityNBT(thrusterSelection, ThrusterBlockEntity.class,
                nbt -> {
    ...
    private static void increaseIntensitySubScene(@NotNull CreateSceneBuilder scene,
                                                  @NotNull Selection throttleSelection,
                                                  CreateSceneBuilder.WorldInstructions world, Selection thrusterSelection) {
        scene.addKeyframe();

        scene.overlay().showControls(
                throttleSelection.getCenter(),
                Pointing.LEFT,
                20
        ).rightClick();
        scene.idle(7);
        ...
        for (int i = 0; i < 15; i++) {
            scene.idle(2);
            int state = i + 1;
            world.modifyBlockEntityNBT(throttleSelection, ThrottleLeverBlockEntity.class,
    ...
    private static void burnOutSubScene(@NotNull CreateSceneBuilder scene,
                                        @NotNull Selection thrusterSelection,
                                        CreateSceneBuilder.@NotNull WorldInstructions world) {
        scene.addKeyframe();
        scene.overlay().showText(60)
                .text("The thruster will continue to fire as long as it has fuel and power and burns out when fuel is empty")
                .pointAt(thrusterSelection.getCenter())
                .placeNearTarget();
